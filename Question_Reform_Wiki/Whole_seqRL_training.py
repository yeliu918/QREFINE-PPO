...
tf.app.flags.DEFINE_integer("epoch", 2, "the epoch number")
tf.app.flags.DEFINE_integer("dec_L", 2, "the decade length")
tf.app.flags.DEFINE_integer("batch_size", 50, "the batch size")
tf.app.flags.DEFINE_integer("num_units", 300, "the number of hidden state units")
tf.app.flags.DEFINE_float("discount_factor", 0.1, "the discount factor of the reward")
tf.app.flags.DEFINE_float("sen_reward_rate", 0.2, "the rate of sentence reward")
...
tf.app.flags.DEFINE_bool("Attention", True, "Attention")

tf.app.flags.DEFINE_string("Data_FileName", "../data/Wiki/wiki_ids_Q10", "The processing data file name")
# huawei: result/Huawei/large_unique0.95_with
# wiki: data/Wiki/wiki_ids_Q10
...
print("\n" + "reward_rate " + str(FLAGS.sen_reward_rate) + " discount_factor " + str(
    FLAGS.discount_factor) + " dec_L " + str(FLAGS.dec_L) + " batch_size " + str(FLAGS.batch_size) + " epoch " + str(
    FLAGS.epoch) + "\n")
