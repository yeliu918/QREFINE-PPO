...
def make_doc_query(query_name):
    # process_file_name = '../data/Wiki/wiki_ids_Q10'

    process_file_name = "Wiki_result_word/language_retrival"

    process_doc = open("../data/Wiki/document_Q10.txt","w")
    process_query = open("../data/Wiki/query_Q10.txt","w")

    f = open(process_file_name, 'rb')
    data = pickle.load(f)
    noisy_sen = data['noisy_sen']
    print "the length of query:", len(noisy_sen)
    clean_sen = data['clean_sen']
    gen_Seq =data['Seq_gen']
    gen_Who = data["Who_gen"]
    answer_sen = data['answer_sen']
    num_data = len(noisy_sen)
...
def retrival_answer(MAX):
    ...
    process_doc = open("../data/Wiki/document_Q10.txt","r")
    ...
    ##search answer
    accuracy=[]
    process_query = open("../data/Wiki/query_Q10.txt","r")
