...
def RL_tuning_model(data_comb, epoch,
    ...
    with G_Seq2Seq.as_default():
        ...
        #                           Embd_train))
        saver_word_rw.restore(sess_word_rw,
                              "/home/ye/PycharmProjects/Qrefine/Seq_ckpt/Wiki/ALL_seq2seq_Bi_" + str(
                                  Bidirection) + "_Att_" + str(
                                  Attention) + "_Emb_" + str(
                                  Embd_train) + "_hiddenunits_" + str(num_units))
    ...
    with G_QA_similiarity.as_default():
        ...
        saver_sen_rw.restore(sess_QA_rw, "/home/ye/PycharmProjects/Qrefine/Seq_ckpt/Wiki/qa_similiarity")
        ...
        for epo in range(epoch):
            ...
            for batch in range(len(noisy_Id) / batch_size):
                    ...
                    for i in range(generated_que.shape[0]):
                        for j in range(seq2seq_len, generated_que.shape[1]):
                            ...
                            if j == generated_que.shape[1] - 1:
                                reward[i][L - 1 - j] = reward[i][L - 1 - j] + sen_reward_rate * (
                ...
                for i in range(generated_que.shape[0]):
                    ...
                    for j in range(seq2seq_len, generated_que.shape[1]):
                        ...
                        if max_index == 2 and label == 0:
                            label = 1
                            RL_rewards[i][j - seq2seq_len] = logits_pro[i, j, max_index] + sen_reward_rate * (
                                QA_similiarity_rd[0][i])
                            break
                ...
                _, rl_reward, word_policy, policy = sess_beamsearch.run(
                    [BeamSearch_seq2seq.RL_train_op,
                     BeamSearch_seq2seq.rl_reward, BeamSearch_seq2seq.word_log_prob, BeamSearch_seq2seq.softmax_policy],
                    fd)
            ...
            for batch_test in range(len(test_noisy_Id) / batch_size):
                ...
                if bleu_score1 > 0.55:
                    ...
                    f.write("\n" + "reward_rate " + str(sen_reward_rate) + " discount_factor " + str(
                        discount_factor) + " dec_L " + str(dec_L) + " batch_size " + str(
                        batch_size) + " epoch " + str(epoch) + "\n")
