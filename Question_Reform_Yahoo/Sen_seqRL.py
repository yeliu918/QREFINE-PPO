...
def RL_tuning_model(Dataset_name, Dataset_name1, data_comb, epoch,
                    batch_size=64,
                    num_units=200,
                    beam_width=5,
                    discount_factor=0.1,
    ...
    char_Id = data_comb[0]['char_Id']
    char_len = data_comb[0]['char_len']
    ...
    val_char_Id = data_comb[1]['char_Id']
    val_char_len = data_comb[1]['char_len']
    ...
    test_Id = data_comb[2]['char_Id']
    test_len = data_comb[2]['char_len']
    ...
    model_name = "BS_"
    values = {}
    checkpoint_path = "/mnt/WDRed4T/ye/Qrefine/ckpt/S2S/" + Dataset_name + "/" + Dataset_name1 + str(Bidirection) + "_Att_" + str(
    ...
    with G_Seq2Seq.as_default():
        ...
        saver_word_rw.restore(sess_word_rw,
                              "/mnt/WDRed4T/ye/Qrefine/ckpt/S2S/" + Dataset_name + "/" + Dataset_name1 + str(Bidirection) + "_Att_" + str(
                                       Attention) + "_Emb_" + str(Embd_train) + "_hiddenunits_" + str(num_units))
    ...
    with G_QA_similiarity.as_default():
        QA_simi_model = QA_similiarity.QA_similiarity(batch_size, 200, embd, model_type)
        ...
        for epo in range(epoch):
            ...
            for batch_test in range(len(test_noisy_Id) / batch_size):
                ...
                if bleu_score1 > 0.538:
                    fname = "/mnt/WDRed4T/ye/Qrefine/" + Dataset_name + "_" + Dataset_name1+ str(bleu_score1)
                    f = open(fname, "wb")
