...
def RL_tuning_model(Dataset_name, Dataset_name1, data_comb, epoch, Char_hidden_units, char_num,  char_dim,
                    batch_size=64,
                    num_units=200,
                    beam_width=5,
                    discount_factor=0.1,
    ...
    model_name = "BS_"
    values = {}
    checkpoint_path = "/mnt/WDRed4T/ye/Qrefine/ckpt/CharS2S/" + Dataset_name + "/" + Dataset_name1 + str(Bidirection) + "_Att_" + str(
    ...
    with G_Seq2Seq.as_default():
        Seq2Seq_model = CharacterBA.Char_Seq2Seq(batch_size, vocab_size, num_units, embd, model_cond,
        ...
        #                           Embd_train))
        saver_word_rw.restore(sess_word_rw,
                              "/mnt/WDRed4T/ye/Qrefine/ckpt/CharS2S/" + Dataset_name + "/" + Dataset_name1 + str(Bidirection) + "_Att_" + str(
                                       Attention) + "_Emb_" + str(Embd_train) + "_hiddenunits_" + str(num_units))
    ...
    with G_QA_similiarity.as_default():
        ...
        saver_sen_rw.restore(sess_QA_rw, "/mnt/WDRed4T/ye/Qrefine/ckpt/" + Dataset_name + "/qa_similiarity")

    G_BeamSearch = tf.Graph()
    with G_BeamSearch.as_default():

        BeamSearch_seq2seq = seq_char_list.BeamSearch_Seq2seq(vocab_size = vocab_size,
                                                         num_units = num_units,
                                                         beam_width = beam_width,
                                                         model_name = model_name, embd=embd,
                                                         Bidirection = Bidirection,
                                                         Embd_train = Embd_train, Attention = Attention,
                                                         Char_hidden_units = Char_hidden_units,
                                                         char_num = char_num,
                                                         char_dim = char_dim,
                                                         max_target_length = L)
        # saver_word_rw = tf.train.Saver()
    ...
    with tf.Session(config=config, graph=G_BeamSearch) as sess_beamsearch:
        ...
        # if os.path.isfile("/home/ye/PycharmProjects/Qrefine/Seq_ckpt/Huawei/ALL_seq2seq_Bi_" + str(Bidirection) + "_Att_" + str(
        #                           Attention) + "_Emb_" + str(
        #                           Embd_train)+"_hiddenunits_" + str(num_units)):
        #     saver = tf.train.Saver()
        #     saver.restore(sess_beamsearch, "/home/ye/PycharmProjects/Qrefine/Seq_ckpt/Huawei/ALL_seq2seq_Bi_" + str(Bidirection) + "_Att_" + str(
        #                           Attention) + "_Emb_" + str(
        #                           Embd_train)+"_hiddenunits_" + str(num_units))
        ...
        for epo in range(epoch):
            ...
            for batch in range(len(noisy_Id) / batch_size):
                ...
                if seq2seq_len < 0:
                    ...
                    fd_qa = {QA_simi_model.answer_inputs: answer_shuffle, QA_simi_model.answer_inputs_length: ans_len,
                    ...
                    QA_similiarity_rd = sess_QA_rw.run([QA_simi_model.two_distance], fd_qa)
                    ...
                    for i in range(generated_que.shape[0]):
                        for j in range(seq2seq_len, generated_que.shape[1]):
                            ...
                            if j == generated_que.shape[1] - 1:
                                reward[i][L - 1 - j] = reward[i][L - 1 - j] + sen_reward_rate * (QA_similiarity_rd[0][i])
                ...
                for i in range(generated_que.shape[0]):
                    ...
                    for j in range(seq2seq_len, generated_que.shape[1]):
                        ...
                        if max_index == 2 and label ==0:
                            label = 1
                            RL_rewards[i][j - seq2seq_len] = logits_pro[i, j, max_index] + sen_reward_rate * (QA_similiarity_rd[0][i])
                            break
                        # if j == generated_que.shape[1] - 1:
            ...
            if bleu_score1 > 0.72:
                fname="Wiki_result/RL_Wiki_bleu1_"+str(bleu_score1)+ "bleu2_"+str(bleu_score2)+ "bleu3_"+str(bleu_score3)+ "bleu4_"+str(bleu_score4)
                f=open(fname, "wb")
                f.write("reward_rate "+str(sen_reward_rate)+" discount_factor "+str(discount_factor)+" dec_L "+str(dec_L)+" batch_size "+str(batch_size)+"epoch"+str(epoch))
                print "the length test set is:",len(generated_test_sen), len(test_answer_sen),len(test_noisy_sen),len(test_clean_sen)
