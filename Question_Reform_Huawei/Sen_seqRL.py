...
def RL_tuning_model(data_comb, epoch,
    ...
    model_name = "BS_"
    values = {}
    checkpoint_path = "/home/ye/PycharmProjects/Qrefine/Seq_ckpt/Huawei/pretrainALL_seq2seq_Bi_" + str(Bidirection) + "_Att_" + str(
                                  Attention) + "_Emb_" + str(
    ...
    with G_Seq2Seq.as_default():
        ...
        #                           Embd_train))
        saver_word_rw.restore(sess_word_rw,
                              "/home/ye/PycharmProjects/Qrefine/Seq_ckpt/Huawei/ALL_seq2seq_Bi_" + str(Bidirection) + "_Att_" + str(
                                  Attention) + "_Emb_" + str(
                                  Embd_train)+"_hiddenunits_" + str(num_units))
    ...
    with tf.Session(config=config, graph=G_BeamSearch) as sess_beamsearch:
        ...
        for epo in range(epoch):
            ...
            for batch_test in range(len(test_noisy_Id) / batch_size):
                ...
                if bleu_score1 > 0.70:
                    fname = "Huawei_result_sen/Huawei_bleu1_" + str(bleu_score1) + "bleu2_" + str(
                        bleu_score2) + "bleu3_" + str(bleu_score3) + "bleu4_" + str(bleu_score4)
                    f = open(fname, "wb")
                    f.write("reward_rate " + str(sen_reward_rate) + " discount_factor " + str(
                        discount_factor) + " dec_L " + str(dec_L) + " batch_size " + str(batch_size) + "epoch" + str(
                        epoch))
