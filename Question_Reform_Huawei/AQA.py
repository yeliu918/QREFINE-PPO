...
class AQA_Seq2Seq(object):
    ...
    def inference(self):
        ...
        with tf.variable_scope("decoder_model"):
            decoder_cell = tf.nn.rnn_cell.BasicLSTMCell(self.hidden_units, state_is_tuple=True)

            # decoder_cell = tf.nn.rnn_cell.MultiRNNCell([decoder_cell] * 4, state_is_tuple=True) ##create the multi-layer cell
            ...
            # self.initial_state = [self.encoder_final_state for i in range(4)]  ##here

            self.initial_state = self.encoder_final_state
            # state_per_layer_list = tf.unstack(self.initial_state, axis=0)   ##create the multi-layer initial_state
...
def next_batch(noisy_Id, noisy_len, input_Id, target_Id, clean_len, batch_size,
               batch_num, idx):
...
def main():
    ...
    if model_type == 'training':
        Seq2Seq_model = AQA_Seq2Seq(batch_size, vocab_size, num_units, embd, model_type, Bidirection, Embd_train, Attention)
        ...
        with tf.Session(config=config) as sess:
            ...
            for epo in range(epoch):
                ...
                for batch in range(len(train_noisy_Id) / batch_size):
                    ...
                    train_shuffle = tf.keras.preprocessing.sequence.pad_sequences(train_shuffle, maxlen=max_word,
                                                                                  padding='post', value=EOS)

                    target_shuffle = tf.keras.preprocessing.sequence.pad_sequences(target_shuffle, maxlen=max_word,
                                                                                   padding='post', value=EOS)
                    # print len(target_shuffle[0])
                    ...
                    if batch == 0 or batch % batches_in_epoch == 0:
                        ...
                        for batch_val in range(len(eval_noisy_Id) / batch_size):
                            ...
                            val_source_shuffle, val_source_len, val_train_shuffle, val_target_shuffle, val_target_len = next_batch(
                                eval_noisy_Id, eval_noisy_len, eval_input_Id, eval_target_Id, eval_clean_len,
                ...
                if epo > 0:
                    ...
                    if patience_cnt > 5:
                        ...
                        saver.save(sess,
                                   "/mnt/WDRed4T/ye/Qrefine/ckpt/AQA/" + Dataset_name +"/" + str(Bidirection) + "_Emb_" + str(Embd_train) + "_hiddenunits_" + str(num_units))
                ...
                if 0.90 > avg_bleu_val > 0.8:
                    saver.save(sess, "/mnt/WDRed4T/ye/Qrefine/ckpt/AQA/" + Dataset_name +"/" + str(Bidirection) + "_Emb_" + str(Embd_train) + "_hiddenunits_" + str(num_units) + "_bleu_" + str(
                        avg_bleu_val))

    elif model_type == 'testing':
        Seq2Seq_model = AQA_Seq2Seq(batch_size, vocab_size, num_units, embd, model_type, Bidirection, Embd_train,
                                     char_hidden_units, Attention, max_char, max_word)
        with tf.Session(config=config) as sess:
            saver_word_rw = tf.train.Saver()
            saver_word_rw.restore(sess, "/home/ye/PycharmProjects/Qrefine/Seq_ckpt/Wiki/222Q_seq2seq_Bi_" + str(
                Bidirection) + "_Att_" + str(Attention) + "_Emb_" + str(Embd_train) + "_hiddenunits_" + str(
                num_units) + "_bleu_0.745747754205")
            ...
            fname = "Huawei_result/Huawei_bleu1_" + str(bleu_score1)
            f = open(fname, "wb")
