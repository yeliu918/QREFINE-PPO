        ...
        with tf.variable_scope("encoder_model"):
            ...
            if self.Bidirection == False:
                ...
                encoder_outputs, self.encoder_final_state = tf.nn.dynamic_rnn(cell=encoder_cell,
                                                                              inputs=self.word_embeddings_cat,
...
def next_batch(noisy_Id, noisy_len, noisy_char_Id, noisy_char_len, input_Id, target_Id, clean_len, batch_size,
               batch_num, idx):
    ...
    return data_shuffle, data_len, char_Id, char_len, train_shuffle, target_shuffle, target_len
...
def main():
    ...
    if model_type == 'training':
        ...
        with tf.Session(config=config) as sess:
            ...
            saver.restore(sess,
                          "/mnt/WDRed4T/ye/Qrefine/ckpt/CharS2S/" + Dataset_name + "/" + Dataset_name1  + "_" + str(Bidirection) + "_Att_" + str(
                                       Attention) + "_Emb_" + str(Embd_train) + "_hiddenunits_" + str(num_units)+ "_bleu_0.491625950768")
            ...
            for epo in range(epoch):
                    ...
                    if batch == 0 or batch % batches_in_epoch == 0:
                        ...
                        for batch_val in range(len(eval_noisy_Id) / batch_size):
                            ...
                            val_source_shuffle, val_source_len, val_char_Id, val_char_len, val_train_shuffle, val_target_shuffle, val_target_len = next_batch(
                                eval_noisy_Id, eval_noisy_len, eval_noisy_char_Id, eval_noisy_char_len, eval_input_Id, eval_target_Id, eval_clean_len,
                ...
                saver.save(sess, "/mnt/WDRed4T/ye/Qrefine/ckpt/CharS2S/" + Dataset_name +"/" + Dataset_name1 + "_" + str(Bidirection) + "_Att_" + str(
                    Attention) + "_Emb_" + str(Embd_train) + "_hiddenunits_" + str(num_units) + "_bleu_" + str(
                    avg_bleu_val))
        ...
        with tf.Session(config=config) as sess:
            ...
            saver_word_rw.restore(sess, "/mnt/WDRed4T/ye/Qrefine/ckpt/CharS2S/" + Dataset_name +"/" + Dataset_name1 + str(Bidirection) + "_Att_" + str(
                        Attention) + "_Emb_" + str(Embd_train) + "_hiddenunits_" + str(num_units) + "_bleu_0.286359797692")
            ...
            for batch in range(max_batches):
                ...
                fd = {Seq2Seq_model.encoder_inputs: source_shuffle, Seq2Seq_model.encoder_inputs_length: source_len,
                      Seq2Seq_model.encoder_char_ids: test_batch_noisy_char_Id, Seq2Seq_model.encoder_char_len:char_len,
                      Seq2Seq_model.dropout_rate: 1}
            ...
            #  len(test_answer_sen)
            fname = "/mnt/WDRed4T/ye/Qrefine/ckpt/CharS2S/" + Dataset_name +"/" + Dataset_name1 + "_bleu1_" + str(bleu_score1)
            f = open(fname, "wb")
