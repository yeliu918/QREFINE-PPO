        ...
        with tf.variable_scope("decoder_model"):
            ...
            if self.model_cond == 'training':
                predicting_decoder = tf.contrib.seq2seq.BeamSearchDecoder(
                    ...
                    output_layer=self.output_layer, length_penalty_weight=0.0)
...
def next_batch(noisy_Id, noisy_len, n_emd, noisy_char_Id, noisy_char_len, input_Id,  target_Id, clean_len, batch_size,
               batch_num, idx):
    ...
    return data_shuffle, data_len, data_nemd, char_Id, char_len, train_shuffle, target_shuffle,  target_len
...
def run_BA(model_type,config):
    ...
    epoch = config.epoch
    seq2seq_ckp_dir = config.seq2seq_ckp_dir
    ...
    if model_type == 'training':
        Seq2Seq_model = Char_Seq2Seq(batch_size, vocab_size, num_units, embd, model_type, Bidirection, Embd_train,
        ...
        with tf.Session() as sess:
            # if os.path.isfile(seq2seq_ckp_dir):
            # print "### load the pretrain"
            # saver.restore(sess, seq2seq_ckp_dir)
            # else:
            sess.run(tf.global_variables_initializer())
            # sess.run(tf.global_variables_initializer())
            # saver.restore(sess, config.seq2seq_ckp_dir)
            ...
            for epo in range(epoch):
                ...
                for batch in range(len(train_noisy_Id) / batch_size):
                    source_shuffle, source_len, source_nemd, char_Id, char_len, train_shuffle, target_shuffle, target_len = next_batch(
                        train_noisy_Id,
                        train_noisy_len,
                        train_nemd,
                        train_noisy_char_Id,
                    ...
                    if batch == 0 or batch % batches_in_epoch == 0:
                        ...
                        for batch_val in range(len(eval_noisy_Id) / batch_size):
                            ...
                            val_source_shuffle, val_source_len, val_nemd, val_char_Id, val_char_len, val_train_shuffle, val_target_shuffle, val_target_len = next_batch(
                                eval_noisy_Id, eval_noisy_len,eval_nemd, eval_noisy_char_Id, eval_noisy_char_len, eval_input_Id, eval_target_Id, eval_clean_len,
                            ...
                            fd_val = {Seq2Seq_model.encoder_inputs: val_source_shuffle, Seq2Seq_model.encoder_inputs_length: val_source_len,
                                      Seq2Seq_model.encoder_char_ids: val_batch_noisy_char_Id, Seq2Seq_model.encoder_char_len: val_char_len,
                ...
                if epo > 0:
                    ...
                    if patience_cnt > 5:
                        ...
                        saver.save(sess, seq2seq_ckp_dir)
                ...
                f.write("the performance: " + seq2seq_ckp_dir + "_epoch_" + str(epo) + "avg_eval_bleu_" + str(avg_bleu_val))
            f.close()
            print "save file"
        ...
        with tf.Session() as sess:
            ...
            saver_word_rw.restore(sess, seq2seq_ckp_dir)
            ...
            for batch in range(max_batches):
                ...
                test_source_shuffle, test_source_len, test_source_nemd, test_char_Id, test_char_len, test_train_shuffle, test_target_shuffle, test_target_len = next_batch(
                    test_noisy_Id,
                    test_noisy_len,
                    test_nemd,
                    test_noisy_char_Id,
                ...
                fd = {Seq2Seq_model.encoder_inputs: test_source_shuffle, Seq2Seq_model.encoder_inputs_length: test_source_len, Seq2Seq_model.encoder_emb: test_source_emd,
                      Seq2Seq_model.encoder_char_ids: test_batch_noisy_char_Id, Seq2Seq_model.encoder_char_len: test_char_len,
                      Seq2Seq_model.dropout_rate: 1}
            ...
            #  len(test_answer_sen)
            fname = seq2seq_ckp_dir + "_bleu1_" + str(bleu_score1)
            f = open(fname, "wb")
