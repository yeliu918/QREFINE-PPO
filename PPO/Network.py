...
class BeamSearch_Seq2seq(object):
    ...
    def training(self):
        with tf.variable_scope(self.model_name + "decoder_model"):
            ...
            self.seq_ids = tf.identity(outputs.sample_id)
    ...
    #     with tf.variable_scope(self.model_name + "decoder_model"):
            ...
            helper_greed = tf.contrib.seq2seq.GreedyEmbeddingHelper(embedding=self.embeddings,
                                                              start_tokens=tf.fill([self.batch_size], 0),
                                                              end_token=1)

            decoder = tf.contrib.seq2seq.BasicDecoder(cell=decoder_cell, helper=helper_greed,
                                                      initial_state=self.initial_state,
                                                      output_layer= self.output_layer)
            outputs, final_state, _ = tf.contrib.seq2seq.dynamic_decode(decoder=decoder, output_time_major=False,
                                                                        impute_finished=True,
    ...
    def PPO_reward(self):
        with tf.variable_scope(self.scope, reuse=None):
            ...
            # def changesize1(old_log_probs_ph,log_prob, advantages_ph, returns_ph):
            #     old_log_probs_ph = tf.slice(old_log_probs_ph, [0, 0, 0],
            #                                      [tf.shape(log_prob)[0], tf.shape(log_prob)[1],
            #                                       tf.shape(log_prob)[2]])
            #     advantages_ph = tf.slice(advantages_ph, [0, 0, 0],
            #                                   [tf.shape(log_prob)[0], tf.shape(log_prob)[1],
            #                                    tf.shape(log_prob)[2]])
            #     returns_ph = tf.slice(returns_ph, [0, 0, 0],
            #                                [tf.shape(log_prob)[0], tf.shape(log_prob)[1],
            #                                 tf.shape(log_prob)[2]])
            #
            #     return old_log_probs_ph, log_prob, advantages_ph,returns_ph
            # def changesize2(old_log_probs_ph,log_prob, advantages_ph, returns_ph):
            #     log_prob = tf.slice(log_prob, [0, 0, 0],
            #                              [tf.shape(old_log_probs_ph)[0], tf.shape(old_log_probs_ph)[1],
            #                               tf.shape(old_log_probs_ph)[2]])
            #     return old_log_probs_ph,log_prob, advantages_ph, returns_ph
            #
            # self.old_log_probs_ph, self.log_prob, self.advantages_ph, self.returns_ph = tf.cond(tf.greater(tf.shape(self.old_log_probs_ph)[1], tf.shape(self.log_prob)[1]),
            #                                                                                     lambda :changesize1(self.old_log_probs_ph,self.log_prob, self.advantages_ph, self.returns_ph),
            #                                                                     lambda: changesize2(self.old_log_probs_ph,self.log_prob, self.advantages_ph, self.returns_ph))
            ...
            #self.value_loss
            self.loss =  - self.policy_loss - self.entropy
